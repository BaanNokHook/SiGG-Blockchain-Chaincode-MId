...
class TimeToSecModel {
    ...
    async Search(stub, dataItem) {
        ...
        let collection = 'TimeToSec';

        let query = {
            selector: {
                TIME_ID: { $regex: dataItem.TIME_ID || '' },
                TIME_NAME: { $regex: dataItem.TIME_NAME || '' },
                IS_ACTIVE: true
            }
        };
    ...
    async Insert(stub, dataItem) {
        ...
        let key = dataItem.TIME_ID;

        let schema = {
            TIME_ID: dataItem.TIME_ID,
            TIME_NAME: dataItem.TIME_NAME,	
            SEC: dataItem.SEC,	
